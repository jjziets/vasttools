...
```
## Hw to move your vast docker driver to a nother drive.
The below guid assumes that you have vastai installed and there is stored data on the current drive. I will provide steps to backup the data and switch to the new drivce. 
```
...
sudo tar -c -I 'pixz -k -1' -f - /var/lib/docker | pv | ./docker.tar.pixz" # this assumes you have eenought space on the OS driver in your home folder to backup the docker driver. We are using pixz as it can de compress using multiple cores. 
...
UUID="92e59ab8-450b-4967-b137-2b81eef5532b" /var/lib/docker/ xfs rw,auto,pquota 0 0   # use the UUID that was generated by xfs_admin above and you need to keep the ""
...
sudo reboot
# After reboot check that the dirver you wanted is mounted to /var/lib/docker and that vastai is running. 
